...
public class ShowDialogueFunction extends Function {

	private IplImage resultImage;
	private int lastState = -1;
	
	public ShowDialogueFunction(Stmt stmt, FunctionTemplate template) {
		super(stmt, template);
	}
	...
	public void calculate(IplImage sourceImage) {
		...
		if (f != null) {
			System.out.println(f.getRetValues().get("count"));
			int count = Integer.parseInt(f.getRetValues().get("count").toString());
			if (lastState != count && count == 0) {
				cvCircle(resultImage, cvPoint((int)(resultImage.width()/2), (int)(resultImage.height()/2)), 100, CvScalar.GREEN, -1, 8, 0);
				JOptionPane.showMessageDialog(VsketchFrame.getInstance(), "Someone has come!");
			} else {
